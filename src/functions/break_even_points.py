...
def break_calc(data_all, ghg_tax):
    ...
    # Axes
    plt.xlabel('Distance (km)', fontsize=axis_font)
    if divisor == 1:
        plt.ylabel('Total Emissions (g CO2)', fontsize=axis_font)
    elif divisor == 1000:
        plt.ylabel('Total Emissions (kg CO2)', fontsize=axis_font)
    elif divisor == 1000000:
        plt.ylabel('Total Emissions (t CO2)', fontsize=axis_font)
    else:
        plt.ylabel('Total Emissions (## UNIT UNCLEAR ##)', fontsize=axis_font)
    plt.xticks(x_ranges, fontsize=ticks_size)
    ...
    plt.grid(True)
    plt.title('Break Even Points Emissions', fontsize=axis_font, fontweight='bold') #style='italic')
    legend = plt.legend(['BEV', 'FCEV', 'PHEV', 'ICEV'], fontsize=legend_font_size, loc='lower right')
    legend1 = plt.legend(['EM_FC_BEV = ' + str(round(fc_emission_raw_list[0], 2)) + ' g CO2 / km',
                          ...
                          'EM_FC_ICEV = ' + str(round(fc_emission_raw_list[3], 2)) + ' g CO2 / km'], fontsize=legend_font_size, loc='upper left')
    ...
    legend_tco = plt.legend(['OPEX_BEV = ' + str(round(opex_list_calculated[0], 3)) + ' € / km', # + ghg_tax
                            ...
                            'OPEX_ICEV = ' + str(round(opex_list_calculated[3], 3)) + ' € / km'], fontsize=legend_font_size, loc = 'upper left')
    plt.title('Break Even Points Costs', fontsize=axis_font, fontweight='bold')
    plt.xlabel('Distance (km)', fontsize=axis_font)
    plt.ylabel('Total Costs (€)', fontsize=axis_font)
    plt.xticks(x_ranges, fontsize=ticks_size)
    plt.yticks(fontsize=ticks_size)
    plt.minorticks_on()
    plt.margins(x=0)
